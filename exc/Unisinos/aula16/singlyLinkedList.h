...
class SinglyLinkedList: public List<T> {
    public:
        ...
        int split(T divisor, SinglyLinkedList<T>& nl); // Exercício 3
};
 ...
 * Retorna 0 indicando a divisão da lista.
 * Retorna 3 se a lista estiver vazia.
 */
template <typename T>
int SinglyLinkedList<T>::split(T divisor, SinglyLinkedList<T>& nl) {
    if(isEmpty())
        return 3;
